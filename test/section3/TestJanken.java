...
//    }
    ...
    void testOutPut() {
        ...
        Referee referee = new Referee("審判タロー");
        JankenResult jankenResult = new JankenResult("グー", "チョキ", player1, player1, player2, referee);
        String expect = """
                審判タロー「じゃんけん・・・ポン！！」
                審判タロー「mikosawaさんの手：グー」
                審判タロー「mokoさんの手：チョキ」
                審判タロー「結果は・・・」
                審判タロー「mikosawaさんの勝利」""";
        String actual = jankenResult.output();
...
//        //オブジェクト生成
//        Janken.main(new String[]{"mikosawa", "moko"});//実行する際の引数
//        Game game = new Game(new Player("mikosawa"), new Player("moko"), new Referee("審判チョコ"));
...
//    }
    ...
    void ジャンケンの判定ができるパーとグーではパーが勝つ() {
        Game game = new Game(new Player("mikosawa"), new Player("mi"), new Referee("審判タロー"));
